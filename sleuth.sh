...
function sendToNotify {
	if [[ -z $1 ]]; then
		...
		if [[ -n $slack_channel ]] && [[ -n $slack_auth ]]; then
			...
			curl -F file=@${1} -F "initial_comment=sleuth zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload 2>>"$LOGFILE" >/dev/null
		fi
	fi
}
